...
const DetailRoomKedokteran = ({ navigation, route }) => {
  // Destructure image from route.params, with a fallback to null
  const { image } = route.params || {}; 
  const defaultImage = detailRuangan1;  // Default image in case no image is passed

  const handleFormRentScreen = () => {
    navigation.navigate("FormRentRoomKedokteran", {
      image,  // Pass the image as a parameter
    });
  };
  ...
  return (
    <ScrollView contentContainerStyle={styles.container}>
      ...
      <Image
        ...
        source={image || defaultImage} // Fallback to defaultImage if image is not provided
      />
